...
def try_stream(graph_app, inputs: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Iterator[Tuple[str, Any]]:
    """
    ...
    try:
        for step in graph_app.stream(inputs, config, stream_mode="updates"):
            yield ("updates", step)
        out = graph_app.invoke(inputs, config)
        yield ("final", out)
        return
    except Exception:
        pass
    ...
    try:
        for step in graph_app.stream(inputs, config, stream_mode="values"):
            yield ("values", step)
        out = graph_app.invoke(inputs, config)
        yield ("final", out)
        return
        ...
        if st.button("📂 Load selected blog"):
            if selected_md_file:
                ...
                if json_file.exists():
                    try:
                        import json
                        saved_state = json.loads(json_file.read_text(encoding="utf-8"))
                        st.session_state["last_out"] = saved_state
                        if "topic" in saved_state:
                            st.session_state["topic_prefill"] = saved_state["topic"]
                    except Exception as e:
                        st.warning(f"Error loading metadata JSON: {e}")
